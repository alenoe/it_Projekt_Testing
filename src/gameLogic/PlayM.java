...
public class PlayM {
	...
	public void isFinished(){
		...
		//Der Schaden wird auf den Gegnern zugef�gt.
			if(p != player){
				p.setDamage(getDamage());
			}
		}
		//Verteilung der Herzeinheiten als Lebenspunkte f�r den Spieler.
			player.getHealed(getHeal());

		//Wenn der Spieler in Tokyo ist, erh�lt er f�r diese Runde seine zwei Bonuspunkte.
		if(player.getPosition() == true){
			player.setVictoryPoints(getVictoryPoints() + 2);
		//Falls nicht erh�lt er seine gew�rfelten Punkte.	
		} else {
